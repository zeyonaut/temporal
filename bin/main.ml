...
(* States satisfying CTL state proposition "psi" in model "model". *)
let rec states_satisfying (model : 'ap temporal_model_t) (psi : 'ap ctl_state_prop_t)
  : States.t
  =
  match psi with
  ...
  | Both (psi_0, psi_1) ->
    States.inter (states_satisfying model psi_0) (states_satisfying model psi_1)
  | Either (psi_0, psi_1) ->
    States.union (states_satisfying model psi_0) (states_satisfying model psi_1)
  ...
  | Necessarily (Perpetually psi) ->
    states_satisfying model (Not (Possibly (Eventually (Not psi))))
  | Necessarily (Eventually psi) ->
    states_satisfying model (Not (Possibly (Perpetually (Not psi))))
  (* A state satisfying "necessarily psi_0 until eventually psi_1" satisfies both "necessarily eventually
     psi_1" and "not possibly not psi_1 until eventually neither psi_0 nor psi_1". *)
  | Necessarily (Until (psi_0, psi_1)) ->
    states_satisfying
      model
      (Both
         ( Necessarily (Eventually psi_1)
         , Not (Possibly (Until (Not psi_1, Not (Both (psi_0, psi_1))))) ))
  | Possibly (Next psi) ->
    States.filter
      (fun state ->
        States.exists (model.can_transition state) (states_satisfying model psi))
      model.states
  ...
  | Possibly (Perpetually psi) ->
    fix
      (fun states_satisfying_possibly_perpetually_psi ->
        States.filter
          (fun state ->
            States.exists
              (model.can_transition state)
              states_satisfying_possibly_perpetually_psi)
          states_satisfying_possibly_perpetually_psi)
  ...
  | Possibly (Until (psi_0, psi_1)) ->
    ...
    fix
      (fun states_satisfying_possibly_psi_0_until_psi_1 ->
        States.union
          (States.filter
             (fun state ->
               States.exists
                 (model.can_transition state)
                 states_satisfying_possibly_psi_0_until_psi_1)
             states_satisfying_psi_0)
          states_satisfying_possibly_psi_0_until_psi_1)
      (states_satisfying model psi_1)
;;

(* Decides if "psi" is valid in "model". *)
let is_valid (model : 'ap temporal_model_t) (psi : 'ap ctl_state_prop_t) : bool =
  (* Assert that the model's transition relation is left-total. *)
  assert (
    States.for_all
      (fun state -> States.exists (model.can_transition state) model.states)
      model.states);
...
print_endline (string_of_bool (is_valid model (Necessarily (Eventually (Label P)))));;

print_endline
  (string_of_bool
     (is_valid model (Necessarily (Eventually (Necessarily (Perpetually (Label P)))))))
